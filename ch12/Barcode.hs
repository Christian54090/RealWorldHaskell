...
encodeDigits s@(first:rest) =
  ...
  where (left, right) = splitAt 5 rest
        ...
        righties = zipWith rightEncode (right ++ [checkDigit s])
...
parseRayPPM =
  ...
  parseNat ==> \w -> skipSpaces ==>&
  parseNat ==> \h -> skipSpaces ==>&
  parseNat ==> \maxValue ->
  assert (maxValue == 255) "max value out of spec" ==>&
  parseByte ==>&
  parseTimes (w*h) parseRGB ==> \pxs ->
  identity (listArray ((0,0),(w-1,h-1)) pxs)
